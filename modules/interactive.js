...
async function sendListMessage(sock, jid, data = {}, options = {}) {
  ...
  const { text = '', footer = '', title, buttonText = 'Select Opsi', sections = [] } = data;

  if (!Array.isArray(sections) || sections.length === 0) {
    throw new InteractiveValidationError('sections is mandatory and must be a non-empty array', {
      context: 'sendListMessage',
      errors: ['sections must be a non-empty array of { title?, rows: [...] }'],
      example: {
        ...
        sections: [{
          title: 'Info & Hiburan',
          rows: [
            { id: 'menu_weather', title: 'Weather', description: 'Check real-time weather' },
            { id: 'menu_gempa', title: 'Gempa', description: 'Info gempa terkini' }
          ]
        }]
      }
    });
  ...
  const buttonParamsJson = JSON.stringify({ title: buttonText, sections });
